...
async function loadMonth(){const key=monthKey(year,month);$('openingLabel').textContent=prevMonthLabel();$('saldoLabel').textContent=`SALDO VED UDGANG AF ${monthNames[month-1].toUpperCase()}`;let res=await sb.from('month_settings').select('*').eq('month_key',key).maybeSingle(); if(res.error){alert('Databasefejl: '+res.error.message);return} settings=res.data; if(!settings){const opening=await getPreviousBalance(); const ins=await sb.from('month_settings').insert({user_id:user.id,month_key:key,year,month,salary_amount:0,opening_balance:opening}).select('*').single(); if(ins.error){alert('Databasefejl: '+ins.error.message);return} settings=ins.data}
